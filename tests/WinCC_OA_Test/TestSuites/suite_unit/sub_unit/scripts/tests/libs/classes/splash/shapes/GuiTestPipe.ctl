/**
  @file $relPath
  @copyright MIT
  @brief Real tests for classes/splash/shapes/GuiTestPipe.ctl.
  @test Verifies the shape wrapper library can create and validate a live PIPE shape.
...
//------------------------------------------------------------------------------
/**
  @brief Real tests for GuiTestPipe.
  @AIgeneratedHelpContent
*/
class TstGuiTestPipe : OaTest
{
//-----------------------------------------------------------------------------
//@public members
//-----------------------------------------------------------------------------
  ...
  */
  ...
  {
    this.assertEqual(testShape.name(), "testPipe",
    ...
    GuiTestPipe wrapper = GuiTestPipe(testShape);
    ...
    // note: the PIPE shape does not support size attributes, so the wrapper will always return 1 for width and height
