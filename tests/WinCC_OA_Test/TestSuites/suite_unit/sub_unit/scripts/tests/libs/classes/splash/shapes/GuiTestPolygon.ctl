/**
  @file $relPath
  @copyright MIT
  @brief Real tests for classes/splash/shapes/GuiTestPolygon.ctl.
  @test Verifies the shape wrapper library can create and validate a live POLYGON shape.
...
//------------------------------------------------------------------------------
/**
  @brief Real tests for GuiTestPolygon.
  @AIgeneratedHelpContent
*/
class TstGuiTestPolygon : OaTest
{
//-----------------------------------------------------------------------------
//@public members
//-----------------------------------------------------------------------------
  ...
  */
  ...
  {
    this.assertEqual(testShape.name(), "testPolygon",
    ...
    GuiTestPolygon wrapper = GuiTestPolygon(testShape);
    ...
    // note: the POLYGON shape does not support size attributes, so the wrapper will always return 1 for width and height
    this.assertEqual(current.value("size.w"), 1,
                     "Expected wrapper to read the width");
    this.assertEqual(current.value("size.h"), 1,
...
//@private members
//-----------------------------------------------------------------------------
  private shape<"POLYGON"> testShape;
};
