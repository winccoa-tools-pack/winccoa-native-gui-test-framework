...
$scriptPath = (Resolve-Path "$repoRoot/OaDevTools/scripts/astyle.ctl").Path
...
proj_path = "$repoRoot/$TestProjectPath/Projects/Global"
...
$mode = if ($ApplyChanges) { "TRUE" } else { "FALSE" }
$stdoutLog = "$repoRoot/.artifacts/ctrl-code-style.stdout.log"
$stderrLog = "$repoRoot/.artifacts/ctrl-code-style.stderr.log"
...
if ($ctrlExit -ne 0) {
  throw "WCCOActrl exited with code $ctrlExit while running style helper."
}

if (-not $ApplyChanges) {
  $changePattern = "formatted|geändert|changed"
  ...
  Write-Host "Detected unformatted CTL files:"
  ...
  throw "Unformatted CTL files detected. Re-run with -ApplyChanges."
}
